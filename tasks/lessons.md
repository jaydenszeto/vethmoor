...
- Compass/camera bearing convention is FROZEN: forward is `(-sin yaw, -cos yaw)`, so a world target's compass bearing is `atan2(-(tx-px), -(tz-pz))` and you face it when `yaw == bearing` (rel 0 → centered tick). N=−z, S=+z, E=+x, W=−x. Pure helpers + tests live in `src/systems/wayfinding.ts`; reuse them rather than re-deriving the signs (easy to mirror by accident).
- Event-driven HUD state (e.g. the objective line via `hud:objective`) only updates when something emits — a Settings toggle that changes its *visibility* (`config.wayfinding`) must re-emit (`game.pushObjective()` from `applySettings`), or the text goes stale while the per-frame polled parts (compass tick) update correctly and the two disagree.

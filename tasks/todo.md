...
- [completed] Soft wayfinding — so players don't feel lost without going full Skyrim. An always-on **objective line** under the compass (the active main-quest stage's short imperative) plus a single **ember tick on the compass** showing the bearing to the nearest stage target — direction only, no floating world marker, no distance. Goes quiet underground, on arrival (within 45 m), when the quest is done, or when toggled off. Data: optional `objective`/`targets` on `QuestStageDef` (main stages 10–60, targets resolve to town/dungeon coords); geometry in pure `src/systems/wayfinding.ts` (bearing convention pinned by tests); `hud:objective` event + per-frame `getObjectiveBearing()` poll; **Settings → Wayfinding hints** toggle (`config.wayfinding`, default on; off = pure Morrowind). Verified live: line + tick render, retarget on stage advance, suppress on arrival, toggle clears both.
